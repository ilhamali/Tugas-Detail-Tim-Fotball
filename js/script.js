const ApiKey = "9d94e9a646f440488e225139d0fb88d3";
        ...
        .then(resJson => {
            ...
            resJson.teams.forEach(team => {
                ...
                <li class="collection-item avatar">
                    ...
                    <a href="#" " btn btn" data-id = "${team.id}" class="secondary-content"><i class="material-icons" data-id = "${team.id}">info</i></a>
                </li>
                `
            });
            ...
            detil.forEach(btn =>{
                btn.onclick = (event) => {
                  showTeamInfo(detailid + event.target.dataset.id);
                }
            })
...
function showTeamInfo(id) {
    //contents.innerHTML = "ini id" + id ;
    fetch(id, fetchHeader)
    .then(response => response.json())
    .then(detil=>{
    contents.innerHTML = ` 
  <div class="col s13 m12">
    <div class="card horizontal">
    <img class="activator" src="${detil.crestUrl}">
      <div class="card-stacked">
        <div class="card-content">
        <h4><b>${detil.name}</b></h4>
        <table>
        <tr>
        <td><b>Berdiri </b></td><td>${detil.founded}</td></tr>
        <tr>
        <td><b>Markas </b></td><td>${detil.venue}</br></td></tr>
        <tr>
        <td><b>Warna Seragam </b></td><td>${detil.clubColors}<br></tr>
        <tr>
        <td><b>Alamat </b></td><td>${detil.address}<br></tr>
        <tr>
        <td><b>Email </b></td><td>${detil.email}<br></tr>
        <tr>
        <td><b>Nomor </b></td><td>${detil.phone}<br></tr>
        <tr>
        <td><b>Website </b></td><td>${detil.website}<br></tr>
        </table>
        </div>
